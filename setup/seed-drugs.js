// ============================================================
//  seed-drugs.js
//  Seed รายการยาเข้า Firestore /drugs collection
//  ใช้ drugCode เป็น document ID เพื่อให้ idempotent (รันซ้ำได้)
//
//  วิธีใช้:
//    node seed-drugs.js
// ============================================================
...
// ── Drug data (sync จาก js/drug-data.js) ─────────────────────
const DRUGS = [
  // ยาแก้ปวด / ลดไข้
...
// ── Main ──────────────────────────────────────────────────────
async function main() {
  console.log('\n💊  PharmBot — Seed Drug Data');
  console.log(`   จำนวน: ${DRUGS.length} รายการ`);
  console.log('─'.repeat(50));

  const batch = db.batch();
  DRUGS.forEach(drug => {
    const ref = db.collection('drugs').doc(drug.drugCode);
    batch.set(ref, {
      ...drug,
  ...
  console.log(`✅  Seed สำเร็จ ${DRUGS.length}/${DRUGS.length} รายการ`);
  console.log('   Firestore collection: /drugs\n');
  process.exit(0);
}
